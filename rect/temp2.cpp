...
Rectangulation::Rectangulation(int n, RectangulationType type, std::vector<RectangulationPattern> &patterns)
    ...
    // M1 - Initialize
    set_all_vertical();
    this->o_.push_back(RectangulationDirection::None);
    this->s_.push_back(-1);
    for (int j = 1; j <= n_; ++j) {
        this->o_.push_back(RectangulationDirection::left);
        this->s_.push_back(j);
    }
}
...
void Rectangulation::set_all_vertical() {
    ...
    edges[2 * this->n_ + 1].init(EdgeDir::Ver, this->n_ + 2, 1, 0, 0, 0, 1,
                                 3);
    edges[3 * this->n_ + 1].init(EdgeDir::Ver, 2 * this->n_ + 2, this->n_ + 1,
                                 0, 0, this->n_, 0, this->n_ + 3);
    for (int i = 2; i <= this->n_; ++i)
        edges[2 * this->n_ + i].init(EdgeDir::Ver, this->n_ + i + 1, i, 0, 0,
                                     i - 1, i, i + 2);
...
void Rectangulation::print_data() {
    ...
    for (auto e : this->edges_) {
        ...
        std::cout << e.tail_ << " " << e.head_ << " " << e.prev_ << " "
                  << e.next_ << " " << e.left_ << " " << e.right_ << " "
                  << e.wall_ << std::endl;
        i++;
    }
    i = 0;
...
void Rectangulation::print_coordinates_diagonal() {
    ...
    for (int i = 1; i < 2 * this->n_ + 3; ++i) {
        ...
        if (v.north_ == 0 && v.east_ == 0 && v.type_ == VertexType::corner)
            TR = i;
        else if (v.south_ == 0 && v.west_ == 0 &&
                 v.type_ == VertexType::corner)
            BL = i;
    }
    assert(BL != -1 && TR != -1);
    int val = 0;
...
bool Rectangulation::next() {
    ...
    if ((j == 1) || ((this->n_ == 2) && (this->type_ == RectangulationType::baligned)))
        return false;
    // M4 - Jump rectangle
    switch (this->type_) {
    case (RectangulationType::generic):
    ...
    if ((this->type_ == RectangulationType::baligned) &&
        (this->o_[j - 1] == RectangulationDirection::left) && is_bottom_based(j - 1)) {
    ...
    if ((this->type_ == RectangulationType::baligned) &&
        (this->o_[j - 1] == RectangulationDirection::right) && is_right_based(j - 1)) {
...
bool Rectangulation::is_right_based(int j) {
    ...
    if (this->edges_[alpha].left_ == 0)
        return true;
    if (this->type_ == RectangulationType::baligned && a == this->rectangles_[j - 1].swest_ &&
        b == this->rectangles_[j - 1].seast_) {
...
void Rectangulation::Sjump(int j, RectangulationDirection d, int alpha) {
    if (d == RectangulationDirection::left) {
        ...
        // this code is obtained by mirroring the case RectangulationDirection::left along the
        // main diagonal Step 1 - Prepare
        const int a = this->rectangles_[j].nwest_;
        const int b = this->rectangles_[j].neast_;
    ...
    while ((this->vertices_[b].type_ != VertexType::bottom) &&
        ...
        while ((this->vertices_[c].type_ != VertexType::right) &&
            ...
            if (this->vertices_[c].type_ == VertexType::top) {
                ...
                int d = c;
                while ((d != b) &&
                       (this->vertices_[d].type_ != VertexType::bottom) &&
    ...
    while ((this->vertices_[b].type_ != VertexType::right) &&
        ...
        while ((this->vertices_[c].type_ != VertexType::bottom) &&
            ...
            if (this->vertices_[c].type_ == VertexType::left) {
                ...
                int d = c;
                while ((d != b) &&
                       (this->vertices_[d].type_ != VertexType::right) &&
                       (this->vertices_[d].type_ != VertexType::corner)) {
                    if (this->vertices_[d].type_ == VertexType::top)
                        return true;
                    const int delta = this->vertices_[d].west_;
